...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 18 15:41:11 2020
...
def Euler(h, f, y_0, start_t, end_t, obsticles, people):
    ...
    n = len(people) + 10 #can change here - time dependent 
    ...
    #fix y_list needs to be a tensor with people, and their history 
        ...
        for j in range(len(people)):
            ...
            people[j].motivation =  (people[j].max_speed * people[j].direction\
                  - people[j].velocity )/people[j].relax_t
            
            people[j].shist.append(people[j].speed)
            
            people[j].dhist.append( [float(people[j].position[0]),float(people[j].position[1])] )
        
        if len(people) < 10: people.append(Agent_setup.Person())
